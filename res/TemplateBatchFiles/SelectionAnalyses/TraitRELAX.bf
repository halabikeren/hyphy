...
{ 
	...
	//parameters.SetStickBreakingDistribution (trait_relax.distribution, trait_relax.ge_guess); // causes internal error - sent issue to SKP. for now, it is commented out. not sure if and why it's related to the error
...
function trait_relax.optimize_null_model() 
{
	...
	trait_relax.null_model.fit = custom_functions.FitLF_FixedBLs (trait_relax.filter_names, trait_relax.trees, None, { "0" : base_model_map}, 
																		...
																		{terms.run_options.proportional_branch_length_scaler: trait_relax.scaler});
...
{	
	...
	model_maps = {};
	model_maps[0] = base_model_map; // here need to give base model map instead 

	// extract the true history from the user provided path
	fscanf(trait_relax.true_history_path, REWIND, "String", treeStr);

	// convert the "FG" labeling into "Test" labeling and "BG" to "Reference" labeling
	true_history_info = trees.ExtractTreeInfo(treeStr);
	custom_functions.dup_data_filter("data_filter_1", &trait_relax.codon_data);
	joint_sequence_filter_names[1] = "data_filter_1";
	joint_sequence_trees[1] = true_history_info;
	model_maps[1] = custom_functions.generate_history_rules_map(true_history_info);
	
	trait_relax.set_codon_model();

	trait_relax.alternative_model.fit = custom_functions.FitLF_FixedBLs(joint_sequence_filter_names, joint_sequence_trees, lf_formula, model_maps, trait_relax.alternative_model.initial_parameters, trait_relax.model_object_map, {terms.run_options.proportional_branch_length_scaler: trait_relax.scaler, "custom_lf_formula": TRUE});
...
{	
	...
	/*Export(char_1_model, trait_relax.char_1.model);				// debug - in RELAX.bf, the test model is not an exportable instance. in fact, it's a dictionary - and not a model instance, as shown here...
	fprintf(stdout, "char_1_model:\n", char_1_model, "\n\n");   // debug */
...
{	
	...
	trait_relax.char_1.model_results = {};
	trait_relax.char_1.model_results["LRT"] = trait_relax.LRT;
	trait_relax.char_1.model_results[terms.trait_relax.k] = trait_relax.fitted.K;
	trait_relax.char_1.model_results[terms.trait_relax.p] = trait_relax.p;
	trait_relax.char_1.model_results["codon sites to trait association"] = sites_bayes_measurements;

	trait_relax.json [terms.json.char_1_results] = trait_relax.char_1.model_results;
